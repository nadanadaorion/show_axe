...
describe.skipIf(!config)('Realtime propagation against a real Supabase instance', () => {
  ...
  it('8. delivers an INSERT on orion_shows to a second client subscribed via Realtime', async () => {
    const id = uniqueId('rt-show')
    const slug = uniqueId('rt-slug')

    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('Timed out waiting for Realtime INSERT event')), 15_000)
      channel = listener
        ...
        .subscribe((status) => {
          ...
          // Only write once the channel is actually listening, to avoid a race.
