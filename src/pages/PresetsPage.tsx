...
function CreatePresetModal({ open, onClose, shows, onCreate }: { open: boolean; onClose: () => void; shows: { id: string; name: string }[]; onCreate: (showId: string, name?: string) => void }) {
  ...
  return <Modal open={open} title="Crear preset desde show" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={!showId} onClick={() => onCreate(showId, name.trim() || undefined)}>Crear</Button></>}><div className="space-y-4"><div><Label>Show de origen</Label><Select value={showId} onChange={(event) => { setShowId(event.target.value); const show = shows.find((item) => item.id === event.target.value); if (show) setName(`${show.name} — preset`) }}><option value="">Selecciona un show</option>{shows.map((show) => <option key={show.id} value={show.id}>{show.name}</option>)}</Select></div><div><Label>Nombre</Label><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre del preset" /></div><p className="text-xs muted">Se copiarán categorías, equipo, personas, horarios, tipo y nota. El estado de verificación del equipo se reinicia.</p></div></Modal>
}
...
function EditPresetModal({ open, onClose, preset, onSave }: { open: boolean; onClose: () => void; preset: { name: string; description?: string; showType?: string; note?: string }; onSave: (patch: { name: string; description?: string; showType?: string; note?: string }) => void }) {
  ...
  return <Modal open={open} title="Editar preset" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={!name.trim()} onClick={() => onSave({ name: name.trim(), description: description.trim() || undefined, showType: showType.trim() || undefined, note: note.trim() || undefined })}>Guardar</Button></>}><div className="space-y-4"><div><Label>Nombre</Label><Input value={name} onChange={(event) => setName(event.target.value)} /></div><div><Label>Descripción</Label><Textarea value={description} onChange={(event) => setDescription(event.target.value)} /></div><div><Label>Tipo de show</Label><Input value={showType} onChange={(event) => setShowType(event.target.value)} /></div><div><Label>Nota base</Label><Textarea value={note} onChange={(event) => setNote(event.target.value)} /></div></div></Modal>
}
...
function ApplyToShowModal({ open, onClose, presetName, shows, onApply }: { open: boolean; onClose: () => void; presetName: string; shows: { id: string; name: string }[]; onApply: (showId: string, mode: 'merge' | 'replace') => void }) {
  ...
  return <Modal open={open} title={`Aplicar “${presetName}”`} onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={!showId} onClick={() => onApply(showId, mode)}>Aplicar</Button></>}><div className="space-y-4"><div><Label>Show de destino</Label><Select value={showId} onChange={(event) => setShowId(event.target.value)}><option value="">Selecciona un show</option>{shows.map((show) => <option key={show.id} value={show.id}>{show.name}</option>)}</Select></div><div><Label>Modo</Label><Select value={mode} onChange={(event) => setMode(event.target.value as 'merge' | 'replace')}><option value="merge">Fusionar con el contenido actual</option><option value="replace">Reemplazar Equipo, Personas y Horarios</option></Select></div></div></Modal>
}
