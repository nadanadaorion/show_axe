...
export default function PresetsPage() {
  ...
  return <>
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_auto]"><SearchInput value={search} onChange={setSearch} placeholder="Buscar presets…" /><div className="flex border-2 border-[var(--strong-line)] bg-[var(--panel)] p-1"><button onClick={() => setArchived(false)} className={`px-3 py-2 font-mono text-[10px] font-bold uppercase ${!archived ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'muted'}`}>Activos</button><button onClick={() => setArchived(true)} className={`px-3 py-2 font-mono text-[10px] font-bold uppercase ${archived ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'muted'}`}>Archivados</button></div></div>
