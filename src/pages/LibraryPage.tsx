...
import { Badge, Button, EmptyState, Input, Label, Modal, PageHeader, SearchInput, Select, Textarea } from '../components/ui'
...
export default function LibraryPage() {
  ...
  return <>
    ...
    <div className="mb-5 overflow-x-auto"><div className="flex min-w-max gap-1 rounded-xl border border-[var(--line)] bg-[var(--panel)] p-1">{sections.map((item) => <button key={item.key} onClick={() => { setSection(item.key); setSearch('') }} className={`rounded-lg px-3 py-2 text-sm font-medium ${section === item.key ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'muted hover:text-[var(--text)]'}`}>{item.label}</button>)}</div></div>
    <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_auto]"><SearchInput value={search} onChange={setSearch} placeholder={`Buscar ${currentSection.label.toLocaleLowerCase()}…`} /><div className="flex rounded-xl border border-[var(--line)] bg-[var(--panel)] p-1"><button onClick={() => setShowArchived(false)} className={`rounded-lg px-3 py-2 text-sm ${!showArchived ? 'bg-[var(--panel-2)] font-medium' : 'muted'}`}>Activos</button><button onClick={() => setShowArchived(true)} className={`rounded-lg px-3 py-2 text-sm ${showArchived ? 'bg-[var(--panel-2)] font-medium' : 'muted'}`}>Archivados</button></div></div>
    <div className="mb-5 rounded-xl border border-[var(--line)] bg-[var(--panel-2)] px-4 py-3 text-sm muted">{currentSection.description}</div>
...
function LibraryItemModal({ open, section, item, library, onClose, onSave }: { open: boolean; section: LibrarySection; item?: LibraryItem; library: ReturnType<typeof useAppStore.getState>['library']; onClose: () => void; onSave: (data: Record<string, unknown>) => void }) {
  ...
  return <Modal open={open} title={`${item ? 'Editar' : 'Nuevo'} ${sections.find((entry) => entry.key === section)?.label.toLocaleLowerCase()}`} onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button type="submit" form="library-item" disabled={!name.trim()}>Guardar</Button></>}><form id="library-item" onSubmit={submit} className="space-y-4"><div><Label>Nombre *</Label><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} /></div>
    {section === 'equipment' && <div className="grid gap-4 sm:grid-cols-2"><div><Label>Categoría</Label><Select value={categoryId} onChange={(event) => setCategoryId(event.target.value)}><option value="">Sin categoría</option>{library.categories.filter((category) => !category.archived).sort((a, b) => a.order - b.order).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</Select></div><div><Label>Unidad</Label><Input value={unit} onChange={(event) => setUnit(event.target.value)} placeholder="pzas, metros, kits…" /></div><div className="sm:col-span-2"><Label>Origen predeterminado</Label><Select value={originId} onChange={(event) => setOriginId(event.target.value)}><option value="">Sin origen</option>{library.origins.filter((origin) => !origin.archived).map((origin) => <option key={origin.id} value={origin.id}>{origin.name}</option>)}</Select></div></div>}
    {section === 'people' && <div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Label>Empresa</Label><Input value={company} onChange={(event) => setCompany(event.target.value)} /></div><div><Label>Tipos</Label><MultiSelect items={library.personTypes.filter((entry) => !entry.archived)} selected={typeIds} onChange={setTypeIds} /></div><div><Label>Funciones</Label><MultiSelect items={library.roles.filter((entry) => !entry.archived)} selected={roleIds} onChange={setRoleIds} /></div><div><Label>Teléfonos</Label><Textarea value={phones} onChange={(event) => setPhones(event.target.value)} placeholder="Uno por línea" /></div><div><Label>Correos</Label><Textarea value={emails} onChange={(event) => setEmails(event.target.value)} placeholder="Uno por línea" /></div></div>}
    <div><Label>Notas</Label><Textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></div></form></Modal>
}
