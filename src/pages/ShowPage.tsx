...
import { Badge, Button, EmptyState, Input, Label, Modal, ProgressBar, SearchInput, Select, Textarea } from '../components/ui'
...
export default function ShowPage() {
  ...
  return (
    <>
      ...
      <Modal open={presetOpen} title="Crear preset desde este show" onClose={() => setPresetOpen(false)} footer={<><Button variant="secondary" onClick={() => setPresetOpen(false)}>Cancelar</Button><Button disabled={!presetName.trim()} onClick={() => { createPresetFromShow(show.id, presetName); setPresetOpen(false) }}>Crear preset</Button></>}><Label>Nombre del preset</Label><Input value={presetName} onChange={(event) => setPresetName(event.target.value)} autoFocus /></Modal>
      ...
      {categories.map((category, index) => {
        ...
        return <section key={category.id} className="panel overflow-hidden" onDragOver={(event) => event.preventDefault()} onDrop={(event) => handleDrop(event, category.id)}>
          <div className="flex flex-col gap-3 border-b border-[var(--line)] bg-[var(--panel-2)] px-4 py-3 sm:flex-row sm:items-center">
            <div className="min-w-0 flex-1"><Input value={category.name} onChange={(event) => updateShowCategory(show.id, category.id, { name: event.target.value })} className="border-transparent bg-transparent p-0 font-semibold focus:border-transparent focus:shadow-none" /><div className="mt-2 max-w-xs"><ProgressBar value={percent} /></div></div>
          ...
          {!items.length ? <div className="p-5 text-sm muted">{search ? 'No hay coincidencias en esta categoría.' : 'Arrastra equipo aquí o agrega un elemento.'}</div> : <div className="divide-y divide-[var(--line)]">{items.map((item) => <EquipmentRow key={item.id} show={show} item={item} origins={library.origins.filter((origin) => !origin.archived).map((origin) => origin.name)} onUpdate={(patch) => updateEquipment(show.id, item.id, patch)} onDropItem={(draggedId) => moveEquipment(show.id, draggedId, category.id, item.order)} onDuplicate={() => duplicateEquipment(show.id, item.id)} onDelete={() => { const removed = deleteEquipment(show.id, item.id); if (removed) showToast('Equipo eliminado', { onAction: () => restoreEquipment(show.id, removed) }) }} />)}</div>}
        </section>
      })}
    </div>
...
function EquipmentRow({ item, show, origins, onUpdate, onDropItem, onDuplicate, onDelete }: { item: ShowEquipmentItem; show: Show; origins: string[]; onUpdate: (patch: Partial<ShowEquipmentItem>) => void; onDropItem: (draggedId: string) => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <div draggable onDragStart={(event) => { event.dataTransfer.setData('text/equipment-id', item.id); event.dataTransfer.effectAllowed = 'move' }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const draggedId = event.dataTransfer.getData('text/equipment-id'); if (draggedId && draggedId !== item.id) onDropItem(draggedId) }}>
    <div className="flex items-center gap-2 px-3 py-3 sm:px-4">
      <GripVertical size={16} className="cursor-grab muted" />
    ...
    {expanded && <div className="grid gap-4 border-t border-[var(--line)] bg-[var(--panel-2)] p-4 sm:grid-cols-2 lg:grid-cols-4">
      <div><Label>Nombre</Label><Input value={item.name} onChange={(event) => onUpdate({ name: event.target.value })} /></div>
      <div className="grid grid-cols-2 gap-2"><div><Label>Cantidad</Label><Input type="number" min="0" step="1" value={item.quantity} onChange={(event) => onUpdate({ quantity: Number(event.target.value) })} /></div><div><Label>Unidad</Label><Input value={item.unit || ''} onChange={(event) => onUpdate({ unit: event.target.value || undefined })} placeholder="pzas" /></div></div>
      <div><Label>Origen</Label><Input list={`origins-${item.id}`} value={item.originName || ''} onChange={(event) => onUpdate({ originName: event.target.value || undefined })} placeholder="Propio, renta…" /><datalist id={`origins-${item.id}`}>{origins.map((origin) => <option key={origin}>{origin}</option>)}</datalist></div>
      <div><Label>Categoría</Label><Select value={item.categoryId} onChange={(event) => onUpdate({ categoryId: event.target.value })}>{[...show.equipmentCategories].sort((a, b) => a.order - b.order).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</Select></div>
      ...
      <div className="sm:col-span-2 lg:col-span-4"><Label>Notas</Label><Textarea value={item.notes || ''} onChange={(event) => onUpdate({ notes: event.target.value || undefined })} /></div>
...
function PersonRow({ person, canMoveUp, canMoveDown, onMove, onUpdate, onDelete }: { person: ShowPerson; canMoveUp: boolean; canMoveDown: boolean; onMove: (direction: -1 | 1) => void; onUpdate: (patch: Partial<ShowPerson>) => void; onDelete: () => void }) {
  const [expanded, setExpanded] = useState(false)
  return <div><div className="flex items-center gap-2 px-4 py-4"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--panel-2)] text-sm font-semibold">{person.name.split(' ').slice(0, 2).map((part) => part[0]).join('').toUpperCase()}</div><button className="min-w-0 flex-1 text-left" onClick={() => setExpanded(!expanded)}><div className="truncate font-medium">{person.name}</div><div className="truncate text-xs muted">{[person.company, ...person.roleNames].filter(Boolean).join(' · ') || 'Sin función'}</div></button><Button variant="ghost" size="icon" disabled={!canMoveUp} onClick={() => onMove(-1)} aria-label="Subir persona"><ChevronUp size={15} /></Button><Button variant="ghost" size="icon" disabled={!canMoveDown} onClick={() => onMove(1)} aria-label="Bajar persona"><ChevronDown size={15} /></Button><Button variant="ghost" size="icon" onClick={() => setExpanded(!expanded)}>{expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</Button></div>
    {expanded && <div className="grid gap-4 border-t border-[var(--line)] bg-[var(--panel-2)] p-4 sm:grid-cols-2">
      <div><Label>Nombre</Label><Input value={person.name} onChange={(event) => onUpdate({ name: event.target.value })} /></div><div><Label>Empresa</Label><Input value={person.company || ''} onChange={(event) => onUpdate({ company: event.target.value || undefined })} /></div>
      <div><Label>Tipos</Label><Input value={person.typeNames.join(', ')} onChange={(event) => onUpdate({ typeNames: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) })} placeholder="Equipo, proveedor…" /></div><div><Label>Funciones</Label><Input value={person.roleNames.join(', ')} onChange={(event) => onUpdate({ roleNames: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) })} placeholder="Manager, audio…" /></div>
      <div><Label>Teléfonos</Label><Textarea value={person.phones.join('\n')} onChange={(event) => onUpdate({ phones: event.target.value.split('\n').map((value) => value.trim()).filter(Boolean) })} placeholder="Uno por línea" /></div><div><Label>Correos</Label><Textarea value={person.emails.join('\n')} onChange={(event) => onUpdate({ emails: event.target.value.split('\n').map((value) => value.trim()).filter(Boolean) })} placeholder="Uno por línea" /></div>
      <div className="sm:col-span-2"><Label>Notas</Label><Textarea value={person.notes || ''} onChange={(event) => onUpdate({ notes: event.target.value || undefined })} /></div><div className="sm:col-span-2"><Button variant="danger" size="sm" onClick={onDelete}><Trash2 size={14} />Eliminar</Button></div>
    </div>}</div>
}
...
function AddPersonModal({ open, onClose, show, onAdd }: { open: boolean; onClose: () => void; show: Show; onAdd: (input: Partial<ShowPerson> & { name: string }, libraryId?: string) => void }) {
  ...
  return <Modal open={open} title="Agregar persona" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={mode === 'library' ? !libraryId : !name.trim()} onClick={() => { if (mode === 'library') { const person = library.people.find((item) => item.id === libraryId); if (person) onAdd({ name: person.name }, person.id) } else onAdd({ name: name.trim() }) }}>Agregar</Button></>}>
    <div className="space-y-4"><div className="flex rounded-xl border border-[var(--line)] p-1"><button type="button" onClick={() => setMode('library')} className={`flex-1 rounded-lg px-3 py-2 text-sm ${mode === 'library' ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'muted'}`}>Desde Biblioteca</button><button type="button" onClick={() => setMode('free')} className={`flex-1 rounded-lg px-3 py-2 text-sm ${mode === 'free' ? 'bg-[var(--accent)] text-[var(--accent-text)]' : 'muted'}`}>Creación libre</button></div>
      {mode === 'library' ? <><SearchInput value={search} onChange={setSearch} placeholder="Buscar en Biblioteca…" /><div className="max-h-64 space-y-2 overflow-auto">{catalog.map((person) => <label key={person.id} className={`flex cursor-pointer gap-3 rounded-xl border p-3 ${libraryId === person.id ? 'border-[var(--text)]' : 'border-[var(--line)]'}`}><input type="radio" checked={libraryId === person.id} onChange={() => setLibraryId(person.id)} /><div><div className="text-sm font-medium">{person.name}</div><div className="text-xs muted">{person.company || 'Sin empresa'}</div></div></label>)}{!catalog.length && <div className="rounded-xl border border-dashed border-[var(--line)] p-5 text-center text-sm muted">No hay personas disponibles en la Biblioteca.</div>}</div></> : <div><Label>Nombre</Label><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre completo" /></div>}
    </div>
  </Modal>
}
...
function InfoTab({ show }: { show: Show }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(380px,.9fr)]">
    <section className="panel p-5"><h2 className="mb-5 font-semibold">Información general</h2><div className="grid gap-4 sm:grid-cols-2"><div><Label>Fecha</Label><Input type="date" value={show.date || ''} onChange={(event) => updateShow(show.id, { date: event.target.value || undefined })} /></div><div><Label>Hora</Label><Input type="time" value={show.time || ''} onChange={(event) => updateShow(show.id, { time: event.target.value || undefined })} /></div><div className="sm:col-span-2"><Label>Tipo de show</Label><Input value={show.showType || ''} onChange={(event) => updateShow(show.id, { showType: event.target.value || undefined })} placeholder="Concierto, festival, showcase…" /></div><div className="sm:col-span-2"><Label>Nota general</Label><Textarea className="min-h-48" value={show.note || ''} onChange={(event) => updateShow(show.id, { note: event.target.value || undefined })} placeholder="Venue, accesos, necesidades especiales, contexto general…" /></div></div></section>
...
function ScheduleRow({ item, next, onUpdate, onDelete }: { item: ScheduleItem; next?: ScheduleItem; onUpdate: (patch: Partial<ScheduleItem>) => void; onDelete: () => void }) {
  ...
  return <div><div className="flex items-center gap-3 px-5 py-4"><div className="w-14 text-sm font-semibold tabular-nums">{item.startTime}</div><button className="min-w-0 flex-1 text-left" onClick={() => setExpanded(!expanded)}><div className="truncate text-sm font-medium">{item.name}</div><div className="text-xs muted">{item.endTime ? `Hasta ${item.endTime}` : next ? `Hasta ${next.startTime}` : 'Sin hora final'}{duration ? ` · ${duration}` : ''}</div></button><Button variant="ghost" size="icon" onClick={() => setExpanded(!expanded)}>{expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</Button></div>{expanded && <div className="grid gap-3 border-t border-[var(--line)] bg-[var(--panel-2)] p-4 sm:grid-cols-2"><div className="sm:col-span-2"><Label>Nombre</Label><Input value={item.name} onChange={(event) => onUpdate({ name: event.target.value })} /></div><div><Label>Inicio</Label><Input type="time" value={item.startTime} onChange={(event) => onUpdate({ startTime: event.target.value })} /></div><div><Label>Fin opcional</Label><Input type="time" value={item.endTime || ''} onChange={(event) => onUpdate({ endTime: event.target.value || undefined })} /></div><div className="sm:col-span-2"><Label>Notas</Label><Textarea value={item.notes || ''} onChange={(event) => onUpdate({ notes: event.target.value || undefined })} /></div><div className="sm:col-span-2"><Button variant="danger" size="sm" onClick={onDelete}><Trash2 size={14} />Eliminar</Button></div></div>}</div>
}
...
function AddScheduleModal({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd: (input: Partial<ScheduleItem> & { name: string; startTime: string }) => void }) {
  ...
  return <Modal open={open} title="Nuevo horario" onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancelar</Button><Button disabled={!name.trim() || !startTime} onClick={() => onAdd({ name: name.trim(), startTime, endTime: endTime || undefined })}>Agregar</Button></>}><div className="space-y-4"><div><Label>Nombre</Label><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Ej. Soundcheck" /></div><div className="grid grid-cols-2 gap-4"><div><Label>Inicio</Label><Input type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} /></div><div><Label>Fin opcional</Label><Input type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} /></div></div></div></Modal>
}
