...
function SectionTitle({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle: string }) { return <div className="flex items-center justify-between gap-3 border-b-2 border-[var(--strong-line)] bg-[var(--panel-2)] px-5 py-4"><div className="flex items-center gap-2 text-lg font-black uppercase tracking-[-.025em]">{icon}{title}</div><div className="font-mono text-[10px] uppercase tracking-[.08em] muted">{subtitle}</div></div> }
