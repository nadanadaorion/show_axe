...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return <ToastContext.Provider value={api}>{children}<div className="fixed bottom-4 right-4 z-[70] flex w-[calc(100%-2rem)] max-w-sm flex-col gap-2">{toasts.map((toast) => <div key={toast.id} className="signal-rule flex items-center justify-between gap-3 border-2 border-white bg-black px-4 py-3 text-sm font-bold text-white shadow-[5px_5px_0_var(--accent)] dark:border-black dark:bg-white dark:text-black"><span>{toast.message}</span>{toast.onAction && <Button size="sm" variant="ghost" className="border-white/20 text-inherit hover:bg-white/10 dark:border-black/20 dark:hover:bg-black/10" onClick={() => { toast.onAction?.(); setToasts((current) => current.filter((item) => item.id !== toast.id)) }}>{toast.actionLabel || 'Deshacer'}</Button>}</div>)}</div></ToastContext.Provider>
}
